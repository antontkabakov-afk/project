...
public class CryptoService : ICryptoService
{
    ...
    private const string AssetHistoryCacheKeyPrefix = "coingecko-asset-history";
    ...
    private static readonly TimeSpan AssetHistoryCacheDuration = TimeSpan.FromMinutes(10);
    ...
    public async Task<IReadOnlyList<CryptoAssetPricePointDto>> GetAssetHistoryAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var supportedCoin = _supportedCoins.FirstOrDefault(item =>
            string.Equals(item.Id, assetId, StringComparison.OrdinalIgnoreCase));

        if (supportedCoin is null)
        {
            throw new ArgumentException("Unsupported asset id.", nameof(assetId));
        }
        ...
        var cacheKey = string.Join(
            ':',
            AssetHistoryCacheKeyPrefix,
            supportedCoin.Id,
            normalizedFromUtc.ToString("O"),
            normalizedToUtc.ToString("O"));

        return await _memoryCache.GetOrCreateAsync(
            cacheKey,
            async entry =>
            {
                entry.AbsoluteExpirationRelativeToNow = AssetHistoryCacheDuration;
                ...
                return pricePoints
                    .Select(item => new CryptoAssetPricePointDto(item.Timestamp, item.CurrentPrice))
                    .ToArray();
            })
            ?? [];
    }
    ...
    private async Task<IReadOnlyList<(DateTime Timestamp, decimal CurrentPrice)>> GetHistoricalPricePointsAsync(
        ...
        DateTime toUtc,
        CancellationToken cancellationToken,
        int maxRateLimitAttempts = MaxRateLimitAttempts)
    {
    ...
    private async Task<T> GetRequiredPayloadAsync<T>(
        ...
        CancellationToken cancellationToken,
        int maxAttempts = MaxRateLimitAttempts)
        where T : class
    {
